...
const mapActivityToUiActivity = (activity: Activity): UiActivity => {
  ...
  switch (activity.type) {
    ...
    case 'answer':
      return {
        ...base,
        title: `Responded to "${activity.response?.challenge?.title || 'a challenge'}"`,
    ...
    case 'upvote':
        ...
        const title = upvotedItem ? (upvotedItem as Challenge).title || 'a post' : 'a post';
        return {
            ...base,
            title: `Upvoted a ${upvotedType}: "${title}"`,
            challengeId: activity.challenge?.id || (activity.response?.challenge as Challenge)?.id,
        };
    default:
      return {
        ...base,
...
export const getRecentActivity = async (
  ...
  const res = await getFromApi('/activities', { 'pagination[limit]': limit, sort: 'createdAt:desc' });
